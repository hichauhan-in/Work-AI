...
def _run_once(engine, question: str, use_web, image) -> None:
    result = engine.answer(question, use_web=use_web, image=image)
...
def main() -> int:
    ...
    parser.add_argument("--web", action="store_true", help="Force web fallback on.")
    ...
    if args.interactive:
        ...
        try:
            while True:
                ...
                if not question:
                    continue
                _run_once(engine, question, use_web, None)
    ...
    _run_once(engine, question, use_web, args.image)
    return 0
